...
export const DETECT_REGEX = {
  ...
  CSS_AT_RULE:
    /@(media|keyframes|font-face|import|charset|namespace|supports|document|page|layer|property|container|scope)\b/i,
  CSS_VAR: /--[a-zA-Z0-9-]+\s*:/,

  // JS
  JS_KEYWORD:
    /\b(function|class|let|const|var|if|for|while|switch|return|async|await|import|export|yield|=>|document|window|console)\b/i,
  JS_OPERATOR: /(===?|!==?|&&|\|\||\+\+|--(?![a-zA-Z<])|\*\*|[+\-*/%&|^!~<>?]=?)/,
  ...
  JS_STRONG_KEYWORDS:
    /(?<![.#])\b(function|const|let|return|if|else|while|for|switch|console|window|export|class)\b|(?<![.#])\bvar\b(?!\s*\()|(?<![@])\bimport\b/,

...
function detectFromExtension(ext, trimmedCode, firstK) {
  ...
  if (
    ext === "css" &&
    (DETECT_REGEX.CSS_RULE.test(trimmedCode) ||
      DETECT_REGEX.CSS_AT_RULE.test(firstK) ||
  ...
  if (
    ...
    return "yaml";
  if (ext === "toml" && (DETECT_REGEX.TOML_TABLE.test(firstK) || DETECT_REGEX.TOML_KEY_VALUE.test(firstK)))
    return "toml";
  if (
    (ext === "md" || ext === "markdown") &&
...
function detectFromContent(trimmedCode, firstK) {
  ...
  if (
    ...
    (DETECT_REGEX.MARKDOWN_LIST.test(firstK) || DETECT_REGEX.MARKDOWN_LINK.test(firstK))
  )
    return "md";
  ...
  // 7. Data Configs
  if (
    DETECT_REGEX.YAML_START.test(firstK) ||
    (DETECT_REGEX.YAML_KEY_VALUE.test(firstK) && DETECT_REGEX.YAML_LIST_ITEM.test(firstK))
  )
    return "yaml";
